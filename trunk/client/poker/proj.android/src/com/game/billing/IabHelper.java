...
public class IabHelper {
    ...
    public static final int IABHELPER_ERROR_IllegalState = -1009;
            ...
            public void onServiceConnected(ComponentName name, IBinder service) {
                ...
                try {
                    ...
                    if (response != BILLING_RESPONSE_RESULT_OK) {
                    	message = "Error checking for billing v3 support.";
                    } else {
                    ...
                    if (listener != null) {
                        listener.onIabSetupFinished(new IabResult(response,message));
                    }
    ...
    public void launchPurchaseFlow(Activity act, String sku, int requestCode, OnIabPurchaseFinishedListener listener, String extraData) {
    	try {
            ...
            act.startIntentSenderForResult(pendingIntent.getIntentSender(),
                                           ...
                                           Integer.valueOf(0));
        }catch(IllegalStateCheckException e) {
    ...
    public boolean handleActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        try {
            ...
            logError("IAB returned wrong data");
            result = new IabResult(IABHELPER_BAD_RESPONSE, "IAB returned wrong data");
            if (mPurchaseListener != null) mPurchaseListener.onIabPurchaseFinished(result, null);
            return true;
        }
        ...
        if (resultCode == Activity.RESULT_OK && responseCode == BILLING_RESPONSE_RESULT_OK) {
            ...
            try {
                ...
                // Verify signature
                if (needVerifySignature() && !Security.verifyPurchase(mSignatureBase64, purchaseData, dataSignature)) {
     ...
     * @throws IllegalStateCheckException 
     */
    private Inventory queryInventory(List<String> skusForDetails) throws IabException, IllegalStateCheckException {
        try {
            ...
            if (skusForDetails!=null && skusForDetails.size()> 0) {
                r = querySkuDetails(inv, skusForDetails);
                if (r != BILLING_RESPONSE_RESULT_OK) {
     ...
     * @param skusForDetails as in {@link #queryInventory}
     * @param listener The listener to notify when the refresh operation completes.
     */
    public void queryInventoryAsync(final List<String> skusForDetails,
        ...
        pool.execute(new Runnable() {
            public void run() {
                ...
                try {
                    ...
                    inv = queryInventory(skusForDetails);
                }  catch (IabException ex) {
                    result = ex.getResult();
                } catch (IllegalStateCheckException e) {
					e.printStackTrace();
					result = new IabResult(IABHELPER_ERROR_IllegalState, e.getMessage());
				} catch (Throwable t) {
					t.printStackTrace();
				} finally {
					flagEndAsync();	
     ...
     * @throws IllegalStateCheckException 
     */
    private void consume(String sku,String token) throws IabException, IllegalStateCheckException {
        ...
        try {
            ...
            throw new IabException(IABHELPER_REMOTE_EXCEPTION, "Remote exception while consuming. sku: " + sku, e);
        }
    }
    /**
     ...
     */
	public void consumeAsync(final String purchaseSku,final String purchaseToken) {
		pool.execute(new Runnable() {
			public void run() {
				try {
					flagStartAsync("consume");
					consume(purchaseSku,purchaseToken);
				} catch (IabException ex) {
					ex.printStackTrace();
				} catch (IllegalStateCheckException e) {
					e.printStackTrace();
    ...
    void checkSetupDone(String operation) throws IllegalStateCheckException {
        if (!mSetupDone) {
            logError("Illegal state for operation (" + operation + "): IAB helper is not set up.");
            throw new IllegalStateCheckException("IAB helper is not set up. Can't perform operation: " + operation);
        }
    }
    ...
    void flagStartAsync(String operation) throws IllegalStateCheckException {
        if (mAsyncInProgress) throw new IllegalStateCheckException("Can't start async operation (" +
    ...
    int queryPurchases(Inventory inv) throws JSONException, RemoteException {
        ...
        do {
            ...
            for (int i = 0; i < purchaseDataList.size(); ++i) {
                ...
                if (needVerifySignature() && !Security.verifyPurchase(mSignatureBase64, purchaseData, signature)) {
    ...
    public boolean needVerifySignature() {
    	return mSignatureBase64 != null && mSignatureBase64.length() > 0;
    }
}
